...
FROM MEMBER_M mm INNER JOIN BOOK_LEND_RECORD blr ON mm.USER_ID = blr.KEEPER_ID
...
FROM
	(SELECT bc.BOOK_CLASS_ID AS ClassID, bc.BOOK_CLASS_NAME AS ClassName,YEAR(blr.LEND_DATE) AS [year], COUNT(blr.BOOK_ID) AS CNT
	 FROM BOOK_LEND_RECORD blr INNER JOIN BOOK_DATA bd ON blr.BOOK_ID = bd.BOOK_ID INNER JOIN BOOK_CLASS bc ON bd.BOOK_CLASS_ID = bc.BOOK_CLASS_ID
...
FROM BOOK_DATA bd 
INNER JOIN BOOK_CODE bc ON bc.CODE_ID = bd.BOOK_STATUS 
INNER JOIN BOOK_LEND_RECORD blr ON bd.BOOK_ID = blr.BOOK_ID
INNER JOIN MEMBER_M mm ON blr.KEEPER_ID = mm.USER_ID
INNER JOIN BOOK_CLASS bc1 ON bc1.BOOK_CLASS_ID = bd.BOOK_CLASS_ID
